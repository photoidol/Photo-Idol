...
const isPaidUser = asyncHandler(async (req, res, next) => {
  if (req.user && req.user.paid) {
    next();
  } else {
    res.status(401);
    throw new Error("Unauthorized access. To use this feature, please ensure your account is verified and upgraded to a premium subscription.");
  }
});
...
module.exports = { protect, admin, author, verified, isPaidUser };
