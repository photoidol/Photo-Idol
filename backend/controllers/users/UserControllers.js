...
const sendVerificationEmail = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      message: "An email has been successfully sent. Please check your inbox.",
    });
  } catch (error) {
    console.log(error);
    res.status(500);
...
const updateRole = asyncHandler(async (req, res) => {
  ...
  await user.save();
  res.status(200).json({
    message: `The user's role has been successfully updated to ${role}.`,
  });
});
...
const updateUserStatus = asyncHandler(async (req, res) => {
  ...
  if (!user.isVerified) {
    res.status(400).json({
      message: "Please verify your account before updating the status.",
    });
    return;
  }
