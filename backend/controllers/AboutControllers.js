...
const createAbout = asyncHandler(async (req, res) => {
  ...
  if (totalImageCount >= 3) {
    return res
      .status(400)
      .json({ message: "The maximum limit of 3 images has been reached." });
  }
...
const deleteAbout = asyncHandler(async (req, res) => {
  ...
  if (!about) {
    res.status(404);
    throw new Error(
      "About not found. Please ensure you've provided the correct About information."
    );
  }

  if (
    about.user.toString() !== req.user._id.toString() &&
    req.user.role !== "admin"
  ) {
    res.status(403);
...
const updateAbout = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while updating the About." });
  }
});
/* ----------------- About Images ------------------- */
...
const createAboutLocation = async (req, res) => {
  try {
    ...
    const aboutLocation = new AboutLocation({
      user,
      name,
      address,
      phone,
    ...
    res.status(201).json({
      message: "About Location created successfully",
      data: savedAboutLocation,
    });
  } catch (error) {
    console.error(error);
...
const getAllAboutLocations = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ total: aboutLocations?.length, data: aboutLocations });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error" });
  }
...
const updateAboutLocation = async (req, res) => {
  ...
  try {
    ...
    const updatedAboutLocation = await AboutLocation.findByIdAndUpdate(
      id,
      updates,
      { new: true }
    );
    ...
    res.status(200).json({
      message: "About Location updated successfully",
      data: updatedAboutLocation,
    });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: "Server error" });
...
const deleteAboutLocation = asyncHandler(async (req, res) => {
  ...
  if (!about) {
    res.status(404);
    throw new Error(
      "About location not found. Please ensure you've provided the correct About location information."
    );
  }
  ...
  res
    .status(200)
    .json({ message: "The About location has been successfully deleted." });
});
/* ----------------- About Location ------------------- */
