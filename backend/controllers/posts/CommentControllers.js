...
const createComment = asyncHandler(async (req, res) => {
  ...
  if (isProfane) {
    return res.status(422).json({
      message:
        "Creation failed because the content contains profane words, and your comment cannot be posted due to content guidelines.",
    });
  }
  ...
  res
    .status(201)
    .json({ message: "Comment posted successfully", comment: commentCreate });
});
